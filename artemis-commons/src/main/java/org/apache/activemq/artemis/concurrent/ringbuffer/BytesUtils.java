...
public final class BytesUtils {
   ...
   public static final int PAGE_SIZE = Env.osPageSize();
   ...
   public static long address(final ByteBuffer buffer) {
      //to avoid adding sun.nio.ch* to OSGI dep
   ...
   public static int alignedIndex(ByteBuffer directByteBuffer, int index, int pow2alignment) {
      ...
      final long alignedAddress = align(address, pow2alignment);
   ...
   public static void zeros(final ByteBuffer buffer, final int offset, final int length) {
      ...
      if (buffer.isDirect()) {
         ...
         address = sun.misc.Unsafe.ARRAY_BYTE_BASE_OFFSET + arrayOffset(buffer) + offset;
         obj = array(buffer);
      }
      UnsafeAccess.UNSAFE.setMemory(obj, address, length, (byte) 0);
   }

   public static boolean unsafeCompareAndSwapLong(Object o, long offset, long expected, long x) {
      return UnsafeAccess.UNSAFE.compareAndSwapLong(o, offset, expected, x);
   }
   ...
   public static void copy(ByteBuffer srcBuffer, int srcIndex, Object dstObj, long dstAddress, int length) {
      ...
      if (srcBuffer.isDirect()) {
         ...
         srcAddress = sun.misc.Unsafe.ARRAY_BYTE_BASE_OFFSET + arrayOffset(srcBuffer) + srcIndex;
   ...
   public static void copy(long srcAddress, Object srcObj, ByteBuffer dstBuffer, long dstIndex, int length) {
      ...
      if (dstBuffer.isDirect()) {
         ...
         dstAddress = sun.misc.Unsafe.ARRAY_BYTE_BASE_OFFSET + arrayOffset(dstBuffer) + dstIndex;
