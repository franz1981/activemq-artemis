...
public class NettyConnection implements Connection {
   ...
   private void closeSSLAndChannel(SslHandler sslHandler, final Channel channel) {
      checkFlushBatchBuffer();
      if (sslHandler != null) {
         try {
            ChannelFuture sslCloseFuture = sslHandler.close();
            sslCloseFuture.addListener(future -> channel.close());
            if (!sslCloseFuture.awaitUninterruptibly(DEFAULT_WAIT_MILLIS)) {
         ...
         if (!closeFuture.awaitUninterruptibly(DEFAULT_WAIT_MILLIS)) {
