...
public class ServerSessionPacketHandler implements ChannelHandler {
   ...
   private final SpscChunkedArrayQueue<Packet> packetsToHandle;

   private final Runnable handlePacketTask;
   ...
   public ServerSessionPacketHandler(final Executor callExecutor,
      ...
      int maxSize = Pow2.MAX_POW2;
      if (this.channel.getConfirmationWindowSize() != -1) {
         maxSize = this.channel.getConfirmationWindowSize();
      }
      this.packetsToHandle = new SpscChunkedArrayQueue<>(1024, maxSize);
      this.handlePacketTask = this::handlePendingPackets;
   }

   private void handlePendingPackets() {
      final SpscChunkedArrayQueue<Packet> packetMessagePassingQueue = this.packetsToHandle;
      Packet packet;
      while ((packet = packetMessagePassingQueue.poll()) != null) {
         this.internalHandlePacket(packet);
      }
   }
   ...
   public void handlePacket(final Packet packet) {
      ...
      //TODO improve this handling, maybe comfirm only after a successfull offer!
      if (!this.packetsToHandle.offer(packet)) {
         throw new IllegalStateException("rejected packet: " + packet);
      }
      if (!this.packetsToHandle.isEmpty()) {
         this.callExecutor.execute(this.handlePacketTask);
      }
   }
   ...
   private void slowPathHandlePacket(final Packet packet) {
         ...
         try {
            switch (type) {
               ...
               case CREATE_QUEUE_V2: {
                  ...
                  requiresResponse = request.isRequiresResponse();
                  session.createQueue(request.getAddress(), request.getQueueName(), request.getRoutingType(), request.getFilterString(), request.isTemporary(), request.isDurable(), request.getMaxConsumers(), request.isPurgeOnNoConsumers(), request.isAutoCreated());
                  if (requiresResponse) {
                     response = new NullResponseMessage();
                  }
                  break;
