            ...
            try {
               if (distributedManager.start(DISTRIBUTED_MANAGER_START_TIMEOUT_MILLIS, TimeUnit.MILLISECONDS)) {
                  try {
                     ...
                     LOGGER.debugf(ex, "Errored while trying to get %s lock", nodeId);
                     distributedManager.stop();
                  } catch (TimeoutException timeout) {
                     LOGGER.debugf(timeout, "Timed-out while creating %s lock", nodeId);
                  }
               }
            } catch (ExecutionException e) {
               LOGGER.debugf(e, "Errored while starting distributed manager");
            }
         }
         if (liveLock != null) {
            try {
               if (liveLock.tryLock(blockingCallTimeout, unit)) {
                  return liveLock;
               }
            } catch (UnavailableStateException ex) {
               LOGGER.debugf(ex, "Errored while trying to acquire %s lock", liveLock.getLockId());
               liveLock = null;
               distributedManager.stop();
            }
         }
