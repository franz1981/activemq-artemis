...
public class JdbcLeaseLockTest {
   ...
   private static final String DROP_NODE_MANAGER_TABLE_SQL = "DROP TABLE " + SQL_PROVIDER.getTableName();
   private static final String JDBC_URL = "jdbc:derby:memory:server_lock_db;create=true";
   private Supplier<? extends Connection> connectionFactory;

   private JdbcLeaseLock lock() {
      return lock(TimeUnit.SECONDS.toMillis(10));
   }

   private JdbcLeaseLock lock(long acquireMillis) {
      return new JdbcLeaseLock(UUID.randomUUID().toString(), this.connectionFactory, SQL_PROVIDER.tryLiveLockOnNodeManagerStoreTable(), SQL_PROVIDER.tryReleaseLiveLockOnNodeManagerStoreTable(), SQL_PROVIDER.renewLiveLockOnNodeManagerStoreTable(), SQL_PROVIDER.isLiveLockedOnNodeManagerStoreTable(), SQL_PROVIDER.currentTimestampOnNodeManagerStoreTable(), acquireMillis, 0);
   }

   @Before
   public void createLockTable() throws SQLException {
      //it will be deleted right after being used
      final String driverClass = "org.apache.derby.jdbc.EmbeddedDriver";
      final Driver driver = JdbcSharedStateManager.uncheckedGetDriver(driverClass);
      final Properties emptyProperties = new Properties();
      this.connectionFactory = () -> {
         try {
            final Connection connection = driver.connect(JDBC_URL, emptyProperties);
            if (connection == null) {
               throw new IllegalStateException("the driver: " + driver.getClass() + " isn't able to connect to the requested url: " + JDBC_URL);
            }
            return connection;
      ...
      try (Connection connection = this.connectionFactory.get()) {
         ...
         connection.createStatement().executeUpdate(SQL_PROVIDER.createNodeManagerStoreTable());
         connection.createStatement().executeUpdate(SQL_PROVIDER.insertLiveLockOnNodeManagerStoreTable());
      }
   }

   @After
   public void dropLockTable() throws SQLException {
      try (Connection connection = this.connectionFactory.get()) {
         connection.setAutoCommit(true);
         connection.createStatement().executeUpdate(DROP_NODE_MANAGER_TABLE_SQL);
      }
   }

   @Test
   public void shouldAcquireLock() {
      final JdbcLeaseLock lock = lock();
   ...
   public void shouldNotAcquireLockWhenAlreadyHeldByOthers() {
      ...
      try {
         ...
         final JdbcLeaseLock failingLock = lock();
   ...
   @Test
   public void shouldAcquireLockWhenHolderGotCollected() throws InterruptedException {
      final CountDownLatch acquired = new CountDownLatch(1);
      Thread shortLivedHolder = new Thread(() -> {
         final JdbcLeaseLock lock = lock();
         if (!lock.tryAcquire()) {
            throw new IllegalStateException("holder hasn't acquired the lock");
         }
         acquired.countDown();
      });
      shortLivedHolder.start();
      Assert.assertTrue("main lock not acquired!", acquired.await(1, TimeUnit.MINUTES));
      //help died lock finalizer to be called
      System.gc();
      final LeaseLock lock = lock();
      Assert.assertEquals("backup lock must acquire a died lock", LeaseLock.AcquireResult.Done, lock.tryAcquire(TimeUnit.MINUTES.toMillis(1), LeaseLock.Pauser.sleep(1, TimeUnit.SECONDS), () -> true));
      lock.release();
   }
